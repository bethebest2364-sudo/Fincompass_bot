...
# 7. Дивиденды (Smart-Lab парсинг)
def get_dividends():
    url = "https://smart-lab.ru/q/dividends/"
    try:
        resp = requests.get(url, timeout=10, headers={'User-Agent': 'Mozilla/5.0'})
        # Ищем таблицу с дивидендами
        match = re.search(r'<table class="simple-little-table".*?>(.*?)</table>', resp.text, re.DOTALL)
        if not match:
            return None
        table = match.group(1)
        ...
        if len(rows) < 2:
            return None
        dividends = []
        for row in rows[1:6]:  # первые 5 строк после заголовка
            cols = re.findall(r'<td[^>]*>(.*?)</td>', row, re.DOTALL)
            if len(cols) >= 4:
                ticker = re.sub(r'<.*?>', '', cols[0]).strip()
                amount = re.sub(r'<.*?>', '', cols[1]).strip()
                date = re.sub(r'<.*?>', '', cols[2]).strip()
                dividends.append(f"• {ticker}: {amount} ₽ (дата: {date})")
        return dividends if dividends else None
    except:
        return None

# 8. Акции (Yahoo Finance)
...
def start(message):
    ...
    if is_subscribed(user_id):
        ...
        markup.add(
            ...
            types.InlineKeyboardButton("💎 Дивиденды", callback_data='dividends'),
        ...
        bot.send_message(
            ...
            "Доступные команды:\n"
            "/usd, /eur, /moex, /crypto, /keyrate, /gold, /oil, /dividends, /news, /stock AAPL, /help",
            reply_markup=markup
        )
    else:
...
def callback_query(call):
    if call.data == 'check_sub':
        ...
        data = get_dividends()
        if data:
            text = "💎 *Ближайшие дивиденды:*\n\n" + "\n".join(data)
            bot.send_message(call.message.chat.id, text, parse_mode="Markdown")
        else:
            bot.send_message(call.message.chat.id, "❌ Данные по дивидендам временно недоступны.")
        bot.answer_callback_query(call.id)
    elif call.data == 'news':
        news = get_news()
        if news:
...
def cmd_dividends(message):
    data = get_dividends()
    if data:
        text = "💎 *Ближайшие дивиденды:*\n\n" + "\n".join(data)
        bot.reply_to(message, text, parse_mode="Markdown")
    else:
        bot.reply_to(message, "❌ Данные по дивидендам временно недоступны.")
