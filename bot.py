...
# ---------- ФУНКЦИЯ ДЛЯ ПОЛУЧЕНИЯ ТЕКУЩЕЙ ЦЕНЫ В РУБЛЯХ ----------
...
def callback_query(call):
    ...
    elif call.data.startswith('quick_ticker_'):
        ticker = call.data.split('_')[2]
        # Запоминаем тикер
        set_state(call.from_user.id, 'add_quantity', {'ticker': ticker})
    ...
    elif call.data.startswith('delete_asset_'):
    ...
    elif call.data == 'clear_portfolio':
        markup = types.InlineKeyboardMarkup()
...
@bot.callback_query_handler(func=lambda call: call.data == 'delete_asset_show')
def delete_asset_show(call):
...
# ---------- ОБРАБОТКА ТЕКСТОВЫХ СООБЩЕНИЙ (ДЛЯ ДОБАВЛЕНИЯ) ----------
...
def handle_text(message):
    ...
    if state == 'add_ticker':
        ...
        clear_state(user_id)
        bot.reply_to(message, f"✅ {data['ticker']} добавлен в портфель.")
        show_portfolio(message, user_id)

    else:
        # Если не в процессе добавления, просто игнорируем или даём подсказку
        bot.reply_to(message, "Используйте кнопки для управления ботом или команду /start для главного меню.")

# ---------- ОБРАБОТЧИК КНОПКИ "ПРОПУСТИТЬ" ДЛЯ ЦЕНЫ ----------
@bot.callback_query_handler(func=lambda call: call.data == 'skip_price')
def skip_price(call):
    data = get_state_data(call.from_user.id)
    add_asset(call.from_user.id, data['asset_type'], data['ticker'], data['quantity'], None)
    clear_state(call.from_user.id)
