...
# 5. Инфляция (исправленная, с тремя резервными источниками)
def get_inflation():
    # Первый источник: Росстат XML
    ...
    try:
        ...
        months = root.findall('.//month')
        if months:
            last = months[-1]
            month_name = last.find('name').text.strip()
            infl_value = last.find('infl').text.strip() + '%'
            return {'month': month_name, 'value': infl_value}
    except:
        pass

    # Второй источник: Statbureau API
    try:
        url2 = "https://www.statbureau.org/ru/russia/inflation-api"
    ...
    # Третий источник: страница ЦБ РФ с инфляцией
    try:
        url3 = "https://www.cbr.ru/hd_base/infl/"
        resp3 = requests.get(url3, timeout=10, headers={'User-Agent': 'Mozilla/5.0'})
        match = re.search(r'<td class="[^"]*">(\d{2}\.\d{2}\.\d{4})</td>\s*<td[^>]*>([\d,]+)</td>', resp3.text, re.DOTALL)
        if match:
            date = match.group(1)
            value = match.group(2).replace(',', '.') + '%'
            return {'month': date, 'value': value}
    except:
        pass
...
def get_news():
    ...
    try:
        ...
        # Второй запасной вариант - Finam RSS
        try:
