...
def get_historical_data(asset_type, symbol, days=7):
    # ... (полный код графиков, без изменений) ...
    pass

def generate_chart(data, title, ylabel='Цена', color='blue'):
    # ... (полный код генерации графика, без изменений) ...
    pass
...
def callback_query(call):
    ...
    # ---------- ОТДЕЛЬНЫЕ КОМАНДЫ (валюты, крипта, индексы, сырьё, новости, ставка) ----------
...
def handle_text(message):
    ...
    if state == 'add_ticker':
        ...
        data = get_state_data(user_id)
        # Если пользователь ввёл цену (число)
        try:
