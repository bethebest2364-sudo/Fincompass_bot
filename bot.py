...
# 4. Ключевая ставка ЦБ (парсинг HTML через re)
def get_key_rate():
    url = "https://www.cbr.ru/hd_base/KeyRate/"
    try:
        resp = requests.get(url, timeout=10)
        text = resp.text
        # Ищем строку с таблицей ставок
        match = re.search(r'<table class="data">.*?<tbody>.*?<tr>.*?<td>([\d\.]+)</td>.*?<td>([\d,]+)</td>', text, re.DOTALL)
        if match:
...
# 5. Инфляция (Росстат, парсинг XML)
def get_inflation():
    ...
    try:
        ...
        if months:
            last = months[-1]
            month_name = last.find('name').text
            infl_value = last.find('infl').text + '%'
            return {'month': month_name, 'value': infl_value}
        return None
    except:
        return None

# 6. Финансовые новости (Finam RSS)
def get_news():
