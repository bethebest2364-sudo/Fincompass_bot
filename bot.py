...
def callback_query(call):
    ...
    # ---------- НАЗАД В ГЛАВНОЕ МЕНЮ (ОБНОВЛЁННЫЙ БЛОК) ----------
        ...
        # Отправляем новое сообщение, а не редактируем старое (чтобы работало с графиками)
        bot.send_message(
            call.message.chat.id,
            "✅ Главное меню:\nВыберите нужный раздел.",
            reply_markup=markup
        )
        # Пытаемся удалить сообщение с кнопкой, чтобы не засорять чат
        try:
